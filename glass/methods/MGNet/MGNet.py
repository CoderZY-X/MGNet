...
class MGNet(BasicModelClass):
    def __init__(self, last_module=ASPP):
        ...
        self.merge_layers = nn.ModuleList([SIU(in_dim=in_c) for in_c in (2048, 1024, 512, 256, 64)])
        ...
        self.iter_layer_1 = MyModel(32, 1)
        self.iter_layer_2 = AtrousModule(32, 32)
    ...
    def train_forward(self, data, **kwargs):
        ...
        output = self.refine(x=output[0],logits=output[1],m=0)
    ...
    def test_forward(self, data, **kwargs):
        ...
        tensor  = self.refine(x=output1[0],logits=output1[1],m=0)
